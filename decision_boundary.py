...
class DecisionBoundary:
    ...
    def __init__(self, X: np.ndarray, y: np.ndarray,clf: SVC):
        self.X = X
        self.y = y
        self.clf = clf
    ...
    def non_linear(self,mlxtend=False):
        if not mlxtend:
            # generate the x and y arrays
            x = np.arange(self.X[:, 0].min() - 1, self.X[:, 0].max() + 1, 0.01)
            y = np.arange(self.X[:, 1].min() - 1, self.X[:, 1].max() + 1, 0.01)
            ...
            # plot the graph
            fig: object = plt.figure(figsize=(12, 7))
            # plot the scatter plot
            plt.scatter(self.X[:, 0], self.X[:, 1], c=self.y)
            ...
            plot_decision_regions(self.X, self.y, self.clf, ax=ax)
